...
NdefRecord_t DetectNdefRecordType(unsigned char *pNdefRecord)
{
    ...
    {
        record.recordPayloadSize = ((uint32_t)pNdefRecord[2]<<24) + ((uint32_t)pNdefRecord[3]<<16) + (pNdefRecord[4]<<8) + pNdefRecord[5];
        typeField = 6;
    }
...
unsigned char* GetNextRecord(unsigned char *pNdefRecord)
{
    ...
    if (!(pNdefRecord[0] & NDEF_RECORD_ME_MASK)) {
        ...
        else
        {
            /* ID present ?*/
            if(pNdefRecord[0] & NDEF_RECORD_IL_MASK)
                temp = (pNdefRecord + 7 + pNdefRecord[1] + ((uint32_t)pNdefRecord[2]<<24) + ((uint32_t)pNdefRecord[3]<<16) + (pNdefRecord[4]<<8) + pNdefRecord[5] + pNdefRecord[6]);
            else
                temp = (pNdefRecord + 6 + pNdefRecord[1] + ((uint32_t)pNdefRecord[2]<<24) + ((uint32_t)pNdefRecord[3]<<16) + (pNdefRecord[4]<<8) + pNdefRecord[5]);
        }
    }
    return temp;
}
