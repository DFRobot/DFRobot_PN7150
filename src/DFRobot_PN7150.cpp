...
bool DFRobot_PN7150::scan(String nfcUid)
{
    if (this->scan()) {
        String nfcUidSrt = "";
        for (int i = 0; i < 4; i++) {
...
bool DFRobot_PN7150::scan(void)
{
    // unsigned char cardType = MODE_POLL | TECH_PASSIVE_NFCA;
    unsigned char DiscoveryTechnologies[] = {
    MODE_POLL | TECH_PASSIVE_NFCA,
    MODE_POLL | TECH_PASSIVE_NFCF,
    MODE_POLL | TECH_PASSIVE_NFCB,
    ...
    bool status = NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_INFINITE);
    if ((status == NXPNCI_ERROR) || (Answer[0] != 0x61) || (Answer[1] != 0x05)) return false;
    memcpy(nfcUid, &Answer[13], 4);
    return true;
}

String DFRobot_PN7150::readUid()
{
    if (!scan())
        return "no card!";
    String nfcUidSrt = "";
...
uint8_t DFRobot_PN7150::readData(int block, uint8_t offset)
{
...
uint8_t DFRobot_PN7150::readData(uint8_t* buffer, uint8_t block)
{
    String error = this->readData(block);
    if (error == "no card!" || error == "read error!" || error == "unknown error!"
...
String DFRobot_PN7150::readData(int page)
{
    bool status;
    unsigned char Resp[256];
    unsigned char RespSize;
    /* Authenticate sector with generic keys */
    unsigned char Auth[] = { 0x40, (unsigned char)(page / 4), 0x10, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    /* Read block */
    unsigned char Read[] = { 0x10, 0x30, (unsigned char)page };
    ...
    if ((NFC_ERROR == status) || (Resp[RespSize - 1] != 0))
        return "read error!";
    ...
    for (int i = 0; i < RespSize - 2; i++) {
        blockData[i] = Resp[i + 1];
        if (Resp[i + 1] <= 0x0f) {
            dataSrt += "0";
            dataSrt += String(Resp[i + 1], HEX);
        } else
            dataSrt += String(Resp[i + 1], HEX);
        if (i < RespSize - 3) dataSrt += " ";
    }
    return dataSrt;
}

void DFRobot_PN7150::writeData(int block, uint8_t index, uint8_t data)
{
...
bool DFRobot_PN7150::writeData(int block, uint8_t data[])
{
    bool status;
    unsigned char Resp[256];
    unsigned char RespSize;
    /* Authenticate sector with generic keys */
    unsigned char Auth[] = { 0x40, (unsigned char)(block / 4), 0x10, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    /* Write block */
    unsigned char WritePart1[] = { 0x10, 0xA0, (unsigned char)block };
    unsigned char WritePart2[17] = { 0x10 };
    ...
    if ((status == NFC_ERROR) || (Resp[RespSize - 1] != 0))
        return false;
    for (uint8_t i = 1;i < sizeof(WritePart2);i++) WritePart2[i] = data[i - 1];// Data to be written
...
bool DFRobot_PN7150::NxpNci_ConfigureSettings(void)
{
    ...
    DBG(gNfcController_generation);
    DBG(sizeof(NxpNci_SettingCurrentTS));
    ...
    if (!memcmp(&Answer[8], NxpNci_SettingCurrentTS, sizeof(NxpNci_SettingCurrentTS)) && (gRfSettingsRestored_flag == false)) {
        ...
        if ((Answer[0] != 0x40) || (Answer[1] != 0x02)) return NXPNCI_ERROR;
    }
#endif
...
bool DFRobot_PN7150_I2C::begin(unsigned char mode)
{
    ...
    if (NxpNci_Connect() == NFC_ERROR) {
        DBG("Error: cannot connect to NXPNCI device-----------------\n");
        return false;
    }
    DBG("NxpNci_Connect ok-----------------\n");

    if (NxpNci_ConfigureSettings() == NFC_ERROR) {
        DBG("Error: cannot configure NXPNCI settings\n");
        return false;
    }
    DBG("NxpNci_ConfigureSettings ok-----------------\n");

    _mode = mode;
    if (NxpNci_ConfigureMode() == NFC_ERROR) {
        DBG("Error: cannot configure NXPNCI\n");
        return false;
    }
    DBG("NxpNci_ConfigureMode ok-----------------\n");
...
{
    uint16_t count = 0, temp = 0;
    if (pBuff == NULL) {
        return ERROR;
    }
    // The chunks 36 bytes requirement was not met, but the temporary tests did not find an issue
    while (buffLen > count) {
        temp = ((buffLen - count) > 32) ? 32 : (buffLen - count);
        if (temp != _pWire->requestFrom(_deviceAddr, (uint8_t)temp)) {   // Master device requests size bytes from slave device, which can be accepted by master device with read() or available()
            return ERROR;
        }
        delay(5);
